...
abcz`;
  ...
  test("number of questions answered should be 0", () =>
  {
    const input = `a
b
c`;
    return assertEquals(countQuestions(input), 0);
  });
...
a`;
  ...
  test("sumr of questions answered should be 6", () =>
  {
    const input = `abc
...
b`;
    return assertEquals(sumAllQuestions(input), 6);
  });
}
...
export function simulateSeating(input)
{
  ...
  const canSeePerson = (x, y, dx, dy) =>
  {
    do 
    {
      if (state[getIndex(x, y)] === "#") return true;
      if (state[getIndex(x, y)] === "L") return false;
  ...
  const countNeighbours = (index) =>
  {
    ...
    for (let dx = -1; dx <= 1; dx++)
    {
      ...
      for (let dy = -1; dy <= 1; dy++)
      {
        ...
        if (canSeePerson(nx, ny, dx, dy)) result++;
      }
    }
    return result;
  };
  ...
  do
  {
    ...
    for (const cell of state)
    {
      ...
      else if (cell === "#" && countNeighbours(index) >= 5) { newState.push("L"); changes = true; }
